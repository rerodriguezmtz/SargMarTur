...
#Wilcox_test Nests - Total
wilcox_test(Nkm~Period, data=CmNest)
wilcox_test(Nkm~Period, data=CcNest)

#Wilcox_test Hatchlings - Total
wilcox_test(Nkm~Period, data=CmHat)
wilcox_test(Nkm~Period, data=CcHat)

# Comparison betwen periods for each camp (change the number of site and species)
CcNestCamp<-CcNest[CcNest$Site=="1",]
wilcox_test(Nkm~Period, data=CcNestCamp)

# Plot Nests per year per species
ggerrorplot(TorNest, x = "Camp", y = "Nkm", desc_stat = "mean_ci", color = "Period",
            palette="lancet", size=0.7)+ 
    facet_wrap(~Species,  ncol=1)+
    xlab("Camp")+ ylab(expression(paste("Nests", ~Km^-1*"")))+
    scale_y_log10(limits = c(1,10000), breaks=c(10,100,1000, 10000))+ 
    theme_grey(base_size=12)+ theme(axis.text.x = element_text(angle=90), legend.position = "none") 

# Spearman correlation between nests and hatchlings
Torcorr<-read.csv("DatCorr.csv", header=T) 
str(Torcorr)

#Transform
Torcorr$Species=factor(Torcorr$Species, levels = c("Chelonia mydas", "Caretta caretta"))
Torcorr$Period=factor(Torcorr$Period, levels = c("BS","AS"))
Torcorr$Season=as.factor(Torcorr$Season)

#Plot correlation
Spear <- ggscatter(Torcorr, x = "Nests", y = "Hatchlings",
                color = "Period",  palette="lancet", facet.by = "Species", 
                scales="free")
   
Spear + stat_cor(aes(color = Period),method = "spearman", 
              label.x = 3)
